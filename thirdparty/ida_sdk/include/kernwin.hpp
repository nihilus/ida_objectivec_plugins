...
enum choose_type_t
{
  ...
  chtype_enum_by_value_and_size,// enum (restriction by value & size)
};
...
//
// Returns the delta of lines that the linearray_t had to
// adjust the place by.
// For example, if the place_t has a lnnum of 5,
// but it turns out, upon generating lines, that the number of lines
// for that particular place is only 2, then 3 will be returned.
...
AS_PRINTF(3, 0) char *askfile_cv(
        ...
        const char *format,
        va_list va);
AS_PRINTF(3, 0) char *askfile2_cv(
        bool forsave,
        const char *defdir,
        const char *filters,
        const char *format,
...
typedef void idaapi chooser_cb_ext_t(void *obj, const intvec_t &selection, void *uc);

// command to be added to the form
struct cmd_info_t
{
  qstring name;       // name of the command
  chooser_cb_ext_t *func; // pointer to the command function
  int flags;          // flags
  int icon;           // id of icon
  qstring shortcut;   // shortcut for command
  void *uc;           // user context
public:
  cmd_info_t() : func(NULL), flags(0), icon(-1), uc(NULL) {}
  cmd_info_t(const char *_name, chooser_cb_ext_t *_func, int _flags=0, int _icon=-1, const char * _shortcut = "", void *_uc = NULL) :
    name(_name), func(_func), flags(_flags), icon(_icon), shortcut(_shortcut), uc(_uc)  {}
};
typedef qvector<cmd_info_t> user_cmds_t;
...
// Callback to add commands to chooser popup menu
typedef void idaapi prepare_popup_cmds_t(void *obj, user_cmds_t *user_cmds, const intvec_t &selection);
typedef qvector<prepare_popup_cmds_t *> popup_cbs_t;
...
struct chooser_info_t
{
  ...
  const char *const *popup_names;   // first 5 menu item names (insert, delete, edit, refresh, copy)
  ...
  // function is called every time before context menu is shown to fill user_cmds
  prepare_popup_cmds_t *prepare_popup_cmds;
};
...
// Install new navigation band colorizer. Returns the previous colorizer function
...
enum msg_notification_t
{
  ...
  msg_popup,        // Popup (context) menu is about to appear
                    // Parameters:
                    //   None
                    // ui_add_output_popup() can be used
  ...
  msg_click,        // Click event
                    ...
                    // Returns: int 1-handled, 0-not handled (invoke default handler)
...
struct view_mouse_event_t
{
  ...
  union location_t
  {
    ea_t ea;                      // flat view (i.e., rtype == TCCRT_FLAT)
    const selection_item_t *item; // graph views (i.e., rtype != TCCRT_FLAT).
  ...
  vme_button_t button;      // represents which mouse button was responsible for generating the
                            // event. This field does not care about the current state of the mouse.
};
#endif
...
enum view_notification_t
{
  ...
  view_popup,        // Popup (context) menu about to appear
                     // Parameters:
                     //   TCustomControl *view
                     // add_custom_viewer_popup_item() can be used
                     // to add items to the menu

  view_click,        // Click event
                     // Parameters:
                     //   TCustomControl *view
                     //   const view_mouse_event_t *event
  ...
  view_mouse_over,   // The user moved the mouse over (or out of)
                     // a node or an edge. This is only relevant in
                     // a graph view.
                     //   TCustomControl *view
                     //   const view_mouse_event_t *event
};
...
// Try to cancel an asynchronious exec request.
// Returns true: sucessfully canceled; false: request has already been processed.
...
inline void add_output_popup(char *name, menu_item_callback_t *cb, void *ud )
{
  callui(ui_add_output_popup, name, cb, ud);
}
...
// Create and/or activate dockable modeless form
AS_PRINTF(1, 0) inline TForm *OpenForm_cv(const char *format, int flags, va_list va)
{
...
AS_PRINTF(3, 0) inline char *askfile2_cv(
        bool forsave,
        const char *defdir,
        const char *filters,
        const char *format,
...
//      form     - dialog box as a string
//                 See its format below
//      flags    - combination of FORM_XXX flags (see ui_open_tform)
...
struct form_actions_t
{
  ...
  // wrapper functions to get/set field values of different types
...
/* Format string for AskUsingForm_c()/OpenForm_c()
  ...
  where
        ...
        width - for A, I, T, X: decimal number specifying size of the buffer passed for text input fields.
                 if omitted or <0, assumed to be at least MAXSTR (QMAXPATH for F)

                For 'B': the code generated when the user presses the button (passed to the button callback)
...
// Display a dialog box and wait for the user to input a file name
//      forsave - the entered file name will be used to save file
//      defdir  - default location
//      filters - filters in the format 'description1|wildcard11;wildcards12;...|description2|wildcards21;wildcards22...|...'
//                or the name of file
//      format  - printf() style format string with the question
// This function displays a window with file names present in the directory
// pointed by 'defval'.
...
// Linear address to ascii string
...
// same as above but possibly with some steps skipped
// 'flags' is one of S2EAOPT_XXX
