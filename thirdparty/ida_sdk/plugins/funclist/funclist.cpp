...
void idaapi run(int /*arg*/)
{
  ...
  for ( size_t i=0; i < n; i++ )
  {
    ...
    tinfo_t type;
    qstring decl;
    char long_name[MAXSTR];
    char true_name[MAXSTR];
    asize_t argsize = 0;
    get_entry_name(ord, true_name, sizeof(true_name));
    if ( get_tinfo2(ea, &type) && type.print(&decl, true_name) )
    {
    ...
    else if ( get_long_name(BADADDR, ea, long_name, sizeof(long_name)) != NULL
           && get_true_name(BADADDR, ea, true_name, sizeof(true_name)) != NULL
           && strcmp(long_name, true_name) != 0 )
    {
//      found mangled name
    }
    else
