...
void idaapi assumes(ea_t ea)
{
  ...
  if ( show || Darea->startEA == ea )
  {
    ...
    {
      if ( i == ph.regCodeSreg || i == ph.regDataSreg ) continue;
      if ( prev == NULL || prev->reg(i) != our.reg(i) )
        gen_cmt_line("%-*s assume %s: %a", int(inf.indent-strlen(ash.cmnt)-2), "", ph.regNames[i], our.reg(i));
    }
  }
}
...
void idaapi h8_gen_stkvar_def(char *buf, size_t bufsize, const member_t *mptr, sval_t v)
{
  ...
  if ( is_hew_asm() )
  {
    qsnprintf(buf, bufsize,
                ...
                COLSTR("%s", SCOLOR_DNUM), name, sign, num);
  }
  else
  {
    qsnprintf(buf, bufsize,
