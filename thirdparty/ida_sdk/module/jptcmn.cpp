...
static inline ea_t calc_entry_target(segment_t* seg, ea_t ea, switch_info_ex_t &si)
{
  ...
  switch ( jsize )
  {
    case 1: off =         get_byte(ea);  if ( is_signed ) off = char(off);   break;
    case 2: off =         get_word(ea);  if ( is_signed ) off = int16(off);  break;
    case 4: off =         get_long(ea);  if ( is_signed ) off = int32(off);  break;
    case 8: off = (sval_t)get_qword(ea); if ( is_signed ) off = sval_t(off); break;
  }
  off <<= si.get_shift();
  if ( si.is_subtract() )
    off = -off;
...
static inline void trim_jtable(switch_info_ex_t &si, bool ignore_refs = false)
{
  ...
  while ( ea < end )
  {
    if ( !is_coagulated_addr(ea) )
    {
      ...
      if ( !hasValue(F) || isCode(F) || isTail(F) || (!ignore_refs && hasRef(F)) )
      {
