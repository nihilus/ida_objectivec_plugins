...
{
  return cmd.itype == ARC_b || cmd.itype == ARC_lp || cmd.itype == ARC_bl || cmd.itype == ARC_j || cmd.itype == ARC_jl;
}
...
void idaapi out(void)
{
  ...
  {
    char nbuf[MAXSTR];
    char *name = get_colored_short_name(cmd.ea, callee & ~1, nbuf, sizeof(nbuf));
    if ( name != NULL )
    {
