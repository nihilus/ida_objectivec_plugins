...
inline void add_dxref(ea_t target)
{
  ...
  if ( get_demangled_name(cmd.ea, target, buf, sizeof(buf), inf.short_demnames, DEMNAM_CMNT, 1) != NULL )
    return;
  ...
  if ( decode_prev_insn(cmd.ea) != BADADDR )
  {
    switch ( cmd.itype )
    {
      ...
      case ARC_b:
        {
          int ccode = cmd_cond();
          int ncode = cmd.auxpref & aux_nmask;
    ...
    case ARC_b:
      {
        ...
        if ( ccode == cAL && ncode == aux_nd ) // branch always and no delay slot?
          islast = 1;
        break;
      }
