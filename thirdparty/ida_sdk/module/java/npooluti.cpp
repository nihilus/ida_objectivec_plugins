...
uchar set_parent_object(void)
{
  if ( curClass.super.Name )
  {
    ...
    if ( fmtName(curClass.super.Name, tmpstr, sizeof(tmpstr), fmt_fullname)
      && memcmp(tmpstr, object, sizeof(object)) == 0 )
    {
...
void SetName(ushort name, ea_t ea, ushort access_mode, uval_t number, uchar rmod)
{
  ...
  if ( fmtString(name, sizeof(str) - 1, fmt) )
  {
    if ( !number )
        ...
        case SEG_XTRN:
            ...
            goto BADIDB;
          {
            ...
            switch ( co.type )
            {
              ...
              case CONSTANT_Class: //lint !e525 Negative indentation
                  lcls = co._name;
...
segment_t *_add_seg(int caller)
{
  ...
  {
    if ( startEA > 0x100000 )
      set_selector(sel = cursel++, startEA>>4);
    else
